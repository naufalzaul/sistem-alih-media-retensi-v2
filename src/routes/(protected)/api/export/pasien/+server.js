...
export async function GET({ fetch, cookies }) {
  try {
    const res = await fetch(`${PUBLIC_API_BASE_URL}/api/v2/pasien/export`, {
      headers: {
    ...
    return new Response(buffer, {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="pasien.xlsx"',
      },
    });
  } catch (err) {
    console.error('Export pasien gagal:', err);
