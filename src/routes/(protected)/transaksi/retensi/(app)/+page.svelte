<script>
  ...
  let currentPage = data?.retensi?.page || 1;
  const perPage = 10;
  ...
  $: retensiData = data?.retensi?.data ?? data?.retensi ?? [];

  $: filteredRetensi = Array.isArray(retensiData)
    ? retensiData.filter((item) => {
    ...
    : [];
  $: totalData = data?.retensi?.total || 0;
  ...
  function handlePageChange(page) {
    ...
    goto(`/transaksi/retensi?${params.toString()}`);
  }
  ...
  <Table
    ...
    actions={[
      ...
      },
      {
        ...
        label: "Edit",
        color: "text-gray-600 hover:bg-gray-50",
        href: (row) => `/transaksi/retensi/formulir/${row.ID}`,
      },
  ...
  {#if selectedDeleteModal}
    ...
    >
      <div slot="custom-section">
        <h1>
          Yakin ingin menghapus transaksi retensi? {selectedDeleteModal?.NamaPasien}
        </h1>
      </div>
    </DeleteModal>
  {/if}
