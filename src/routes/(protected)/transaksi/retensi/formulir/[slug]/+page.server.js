...
export function load({ cookies }) {
  ...
  return {
    formOptions: {
      statusOption: ["Belum diretensi", "Sudah diretensi"]
    },
    defaultValues: {
...
export const actions = {
  default: async ({ request, fetch, cookies }) => {
    ...
    const result = retensiSchema.safeParse(data);
    if (!result.success) {
      return fail(400, {
    ...
    try {
      const response = await fetch(`${PUBLIC_API_BASE_URL}/api/v2/retensi/${payload.IdKunjungan}`, {
        method: 'PUT',
        headers: {
      ...
      if (json.status !== 'success') {
        return fail(400, {
          errors: {},
          toast: { type: 'error', message: json.message || 'Simpan retensi gagal' }
        });
      }
      ...
      return {
        success: true,
        toast: { type: 'success', message: 'Data retensi berhasil disimpan' },
        redirect: '/transaksi/retensi'
      };
    } catch (error) {
      return fail(500, {
        errors: {},
