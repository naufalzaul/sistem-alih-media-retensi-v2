<script>
  ...
  const totalAlihMedia = {
    title: "Total Alih Media",
    value: data?.statistik?.total_dokumen || 0,
    change: 0,
  };
  const alihMediaAktif = {
    title: "Alih Media Aktif",
    value: data?.statistik?.total_sudah || 0,
    change: 0,
  };
  const alihMediaNonAktif = {
    title: "Alih Media Non-Aktif",
...
<section class="text-gray-800 mb-12">
  <div>
    <h2 class="text-2xl font-semibold">Statistik Alih Media</h2>
    <p class="text-sm text-gray-500 mb-4">
      Ringkasan total alih media, aktif, dan non-aktif.
    </p>
  </div>

  <div class="my-7 grid grid-cols-1 md:grid-cols-3 gap-4">
    {#each [totalAlihMedia, alihMediaAktif, alihMediaNonAktif] as item, i}
      <div
