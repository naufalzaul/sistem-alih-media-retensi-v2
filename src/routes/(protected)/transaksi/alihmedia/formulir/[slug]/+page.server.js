...
const alihMediaSchema = z.object({
...
export function load({ cookies }) {
  ...
  return {
    formOptions: {
      statusOption: ["Belum dialih media", "Sudah dialih media"]
    },
    defaultValues: {
...
export const actions = {
  default: async ({ request, fetch, cookies }) => {
    ...
    const result = alihMediaSchema.safeParse(data);
    ...
    try {
      const response = await fetch(`${PUBLIC_API_BASE_URL}/api/v2/alih-media/${payload.IdKunjungan}`, {
        method: 'PUT',
        headers: {
      ...
      if (json.status !== 'success') {
        return fail(400, {
          errors: {},
          toast: { type: 'error', message: json.message || 'Simpan alih media gagal' }
        });
      }
      ...
      return {
        success: true,
        toast: { type: 'success', message: 'Data alih media berhasil disimpan' },
        redirect: '/transaksi/alihmedia'
      };
    } catch (error) {
      return fail(500, {
        errors: {},
